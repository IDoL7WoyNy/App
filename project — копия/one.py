...
try:
    connection = pymysql.connect(
        ...
        cursorclass=pymysql.cursors.DictCursor)
    with connection.cursor() as cursor:
        select_all_rows = "SELECT name FROM products WHERE id=1"
        cursor.execute(select_all_rows)
        name_new = cursor.fetchone()
        name = name_new["name"]
    with connection.cursor() as cursor:
        select_all_rows = "SELECT calories FROM products WHERE id=1"
        cursor.execute(select_all_rows)
        calories_new = cursor.fetchone()
        calories = calories_new["calories"]
    with connection.cursor() as cursor:
        select_all_rows = "SELECT masse FROM products WHERE id=1"
        cursor.execute(select_all_rows)
        masse_new = cursor.fetchone()
        masse = masse_new["masse"]
    with connection.cursor() as cursor:
        select_all_rows = "SELECT recipe FROM products WHERE id=1"
        cursor.execute(select_all_rows)
